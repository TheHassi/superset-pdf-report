from .config import PATH, NEXTCLOUD_URL, NEXTCLOUD_USER, NEXTCLOUD_PASSWORD, NEXTCLOUD_FOLDER
...
def transfer_file_to_nextcloud(job_detail):
    ...
    if client.check(NEXTCLOUD_FOLDER) == False:
    ...
    file_name_date = job_detail["filename"].replace(".tex", "_{}.pdf".format(datetime.today().strftime("%d_%m_%Y")))
    try:
        client.upload_sync( remote_path="{}/{}".format( NEXTCLOUD_FOLDER, file_name_date), 
